...
use ethabi::{encode, Address, Uint};
...
fn _domain_separator() -> [u8; 32] {
    ...
    let domain_separator = env::keccak256_array(
        encode(&[
...
fn _hash_linking_accounts_no_domain(linking_accounts: LinkingAccounts) -> [u8; 32] {
    let linking_accounts_type = "LinkingAccounts(LinkingAccount account_a,LinkingAccount account_b,string statement)LinkingAccount(string origin_id,string account_id)";
    ...
    env::keccak256_array(
        encode(&[
            ...
            Token::FixedBytes(env::keccak256_array(linking_accounts.statement.as_bytes()).to_vec()),
        ])
        .as_slice(),
    )
}
...
impl Counter {
    ...
    pub fn eth_verify_eip712(
        &self,
        linking_accounts: LinkingAccounts,
        signature: EcrecoverSigInput,
    ) -> EcrecoverOutput {
